...
public class DOLDACommandLineParser implements CommandLineParserUtil {
	...
	public DOLDACommandLineParser(String [] args) throws ParseException {
		...
		options.addOption( "idcolno", "id_column_no",    true, 
				"Number of the column that contains the id of the observation. Needed to tie togehter text and additional covariates" );
		options.addOption( "idcolnme", "id_column_name",    true, 
				"Name of the column that contains the id of the observation. Needed to tie togehter text and additional covariates" );
		...
		try {
			...
			System.out.println(this.getClass().getSimpleName()  + ": Could not parse command line due to:  " + e.getMessage());
			System.out.println("Args where:");
			for (int i = 0; i < args.length; i++) {
				System.out.print(args[i] + ", ");
			}
			System.out.println();
			formatter.printHelp(this.getClass().getSimpleName()  + " [options] <csv file>", options );
			System.exit(-1);
		}
